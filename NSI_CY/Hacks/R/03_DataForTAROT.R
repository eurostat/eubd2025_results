...
nuts3_shapes <- readRDS(paste0(mapr_path,"NUTS3_Shapefile.rds"))      # Eurostat - NUTS3 mapping from Eurostat
popn_dt <- readRDS(paste0(mapr_path,"Popn_NUTS3_ByAge.rds"))          # Eurostat - Population at NUTS regions from Eurostat
corr_eqn <- readRDS(paste0(corrln_reslts,"CorrExpEqnCoeffs_PMg5.rds"))# Derived - Correlation Equation Coefficients
saveRDS(corr_eqn, paste0(TAROT_supp,"CorrExpEqnCoeffs_PMg5.rds")) 
...
saveRDS(dt_CNTR_names, paste0(TAROT_supp,"NUTS0_geonames_map.rds")) 


# #################################################################################
# [C] IMPORT COPERNICUS DATA
full_paths <- list.files(live_path, full.names = TRUE)
myears <- c()
for(f in 1:length(full_paths)){
  ...
  saveRDS(copern, paste0(TAROT_dats,"Estimate_PremDths_",myear,".rds"))
